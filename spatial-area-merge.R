...
#for entire state
#df<-left_join(df,census_tract[,"code_tract"],by="code_tract")
...
@@ -132,13 +122,13 @@ merge_smallest_by_centroid <- function(df) {
  ...
  @@ -151,49 +141,19 @@ merge_smallest_by_centroid <- function(df) {
  ...
  #compute total income if there are no missing incomes 
  #do missing data imputation if income_total=NA
  ...
  iterate_merging <- function(df, pop_threshold, max_iters =1800) {  
    iterate_merging <- function(df, pop_threshold, max_iters =5500) {  
      ...
      @@ -223,53 +183,37 @@ iterate_merging <- function(df, pop_threshold, max_iters =1800) {
      }
    }
    ...
    #load data
    # aggregated_sp_census_state<-readRDS("aggregated_sp_census_state_3sept.rds") %>%
    ...
    aggregate_cases_by_tract<- function(aggregated_sp_state, weekly_sum){
      ...
      new_merged_weekly_sum<-aggregate_weekly_cases(newdf,weekly_sum)
      ...
      ifelse (aggregated_sp_state$merged_area_bool=="FALSE", 
              a<-(left_join(aggregated_sp_state[!(aggregated_sp_state$merged_area_bool 
                                                  %in% newdf$merged_area_bool),], 
      ...
      return(rbind(a,b))
      @@ -284,10 +228,7 @@ aggregate_weekly_cases<- function(aggregated_sp_state,weekly_sum) {
        "ct9","ct10","ct11","ct12",
        "ct13","ct14","ct15")) %>%
  rename(code_tract=code_tract_original)
newdf_with_summed_cases <-data.frame(code_tract=NA, 
                                     weeklysum=NA, 
                                     week_notific=NA,
                                     status=NA)
...
for (i in 1:nrow(df)){
  cases_per_merged_tract<-filter(weekly_sum, code_tract %in% 
                                   c(df[i,]$ct1,df[i,]$ct2,df[i,]$ct3,
                                     @@ -301,65 +242,52 @@ aggregate_weekly_cases<- function(aggregated_sp_state,weekly_sum) {
...
# test<-readRDS("/Volumes/kebl6022/covid_brazil/cov_socialinequalities/aggregate_final_server.rds")
# dt = as.data.table(aggregate_final)
# 
# dt2<-dt[() & is.na(household_density), 
